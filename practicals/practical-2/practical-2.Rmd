...
And now let's plot the smooth with `plot()`.

```{r gam-plot, exercise=TRUE}
...
Let's add random smooths for `speaker` and `word`.
...
In the previous models we were including `vowel` and `voicing` as factors in `by`-variable smooths. In this way, we are modelling their effect separately. When plotting the estimated smooths, the individual effects `vowel` and `voicing` are added up to each other (hence the 'additive' in generalised additive models).
...
Classic interactions in GAMs (like the ones you add in `lmer` models with the colon `:` notation) can be included using `by`-variables. The factor in the `by`-variable is a interaction factor.

You can create a interaction factor with `interaction()` (the factor needs to be an ordered factor with treatment contrasts, like any other `by`-variable factor). I have already done this for you with the following code.
...
Add the necessary terms in the following exercise.
...
plot_smooths(f0_gam_4, ..., comparison = ..., facet_terms = ..., list(vow_voi = c("vowel", "voicing")))
```
