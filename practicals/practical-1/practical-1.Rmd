...
In this practical, we will build other model incrementally for pedagogical purpuses. In real life applications, you will prepare the data for fitting and start from a model with all of the terms you think are necessary.
...
Let's now plot the model with `plot()`!

```{r plot-gam, exercise=TRUE, exercise.lines=1}
plot(F0_gam)
```

If you want, try different basis and different values of k and plot the result in the following exercise.
...
What we are really interested in is to compare the effect of **speech rate** on F0 in the two focus conditions: *focus* and *non-focus*. We can do this by setting up a model using a `by` variable with an ordered factor. Remember that to use a `by` variable we need to create an ordered factor and change the contrasts to treatment (I did this for you already).
