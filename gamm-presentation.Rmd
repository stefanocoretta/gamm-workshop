...
```{r bam-acf}
acf_plot(resid(vowel_gam), split_by = list(vowels$index))
```

## Residuals autocorrelation

```{r bam-start-event, echo=TRUE}
vowels <- vowels %>%
...
```{r ar-gam, echo=TRUE, warning=FALSE}
...
```{r ar-gam-plot}
plot_smooths(ar_gam, time_point, voicing, vowel, split = list(vow_voi = c("vowel", "voicing")))
```
...
```{r ar-difference}
plot_difference(ar_gam, time_point, list(vow_voi = c("u.voiced", "u.voiceless"))) +
  labs(title = "Difference smooth of /u/ (voiced - voiceless)")
```
